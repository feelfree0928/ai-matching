...
def compute_breakdown(
    ...
    *,
    ...
    Category is now a hard ES filter; it is NOT a scored dimension.
    """
    ...
    # Experience: primary + secondary (expTotal removed — it double-counted secondary years)
    ...
    exp_primary = sigmoid_prim * prim_rel_sq * years_cap
    ...
    # Seniority: default to 2 (mid) when field is absent — prevents missing candidates from
    # appearing as a perfect seniority match (which beat candidates with a small real mismatch).
    cand_lvl = _get_int(doc_source, "seniority_level_int", 2)
