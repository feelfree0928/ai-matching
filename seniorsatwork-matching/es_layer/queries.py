...
def build_script_score_breakdown_field(
    title_vec: list[float],
    industry_vec: list[float],
    skills_vec: list[float],
    edu_vec: list[float],
    ...
    """Build script_fields entry that returns per-dimension scores (same math as script_score).
    Used so the API can show e.g. Title (0.8) × 0.15 for each profile.
    """
    return {
        "script": {
            "source": """
                ...
                return [
                    'title': titleSim,
                    'industry': industrySim,
                    'experience': expScore,
                    'skills': skillsSim,
                    'seniority': seniorityFit * 2.0,
                    'education': eduSim,
                    'language': langScore
                ];
            """,
